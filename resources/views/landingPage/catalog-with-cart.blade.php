...
@section('content')
    ...
    <div class="max-w-7xl mx-auto p-8">
        ...
        {{-- Grid Produk --}}
        <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 pb-10 border-b border-gray-300">

            {{-- Data Produk (Loop produk katalog) --}}
            @php
                $products = [
            ...
            @foreach ($products as $product)
                <div class="bg-white rounded-xl shadow-lg overflow-hidden transition transform hover:shadow-xl duration-300">
                    ...
                    <div class="p-4">
                        ...
                        {{-- Tombol Lihat Detail (yang akan memicu form keranjang di bawah) --}}
                        <button onclick="document.getElementById('cart-section').scrollIntoView({ behavior: 'smooth' })"
                                class="w-full py-2 text-sm text-white font-medium rounded-md shadow-sm" style="background-color: #934c26;">
                            Lihat Detail
                        </button>
                    </div>
                </div>
        ...
        {{-- ======================================================== --}}
        {{-- BAGIAN MEMASUKKAN KERANJANG (Modal/Inline Section) --}}
        {{-- ======================================================== --}}
        <div id="cart-section" class="mt-16 bg-gray-100 p-8 rounded-xl shadow-inner max-w-4xl mx-auto">

            <a href="#katalog" class="text-gray-600 hover:text-gray-900 flex items-center mb-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-8 items-start">
                ...
                {{-- Form Jumlah dan Aksi --}}
                <div class="md:col-span-2 space-y-4">
                    <p class="text-sm text-gray-600 font-semibold">Stok Tersedia: 10</p>
                    ...
                    <form action="{{ route('cart') }}" method="POST" class="mt-6">
                        ...
                        <div class="flex items-center space-x-3">
                            ...
                            <button type="submit" class="px-8 py-2 text-white font-medium rounded-lg shadow-md hover:opacity-90 transition duration-150" style="background-color: #934c26;">
                                Tambah Ke Keranjang
                            </button>
                        </div>
                    </form>
